"""training and validating a ResNet-18 model on the CIFAR-100 dataset using PyTorch"""

import torch
from torch.utils.tensorboard import SummaryWriter  #for logging training progress to TensorBoard
...
device = "cuda" if torch.cuda.is_available() else "cpu" #checks if CUDA (GPU) is available and assigns the device accordingly.
...
#Model
model = torch.hub.load("pytorch/vision:v0.10.0", "resnet18", pretrained=False, num_classes=100).to(device)
